...
高速開発モードP0の改善として、幼稚園「つみき」を文章二択から情景内タップへ変換した。作りかけの町、遊んでいる子、見ている友達、2つの積み木を同じ画面に置き、積み木自体を「わたす」「おく」の44px以上の操作対象にした。既存の行動、採点、待つ選択、結果文は変更していない。残る文章選択式の既存テーマは、同じく情景内タップ / ドラッグ / 待つ型へ順次変換する。
