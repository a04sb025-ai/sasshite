...
const guideKey = 'sasshite-guide-seen-v04'
...
export default function App() {
  ...
  const start = () => {
    ...
    setIndex(0)
    setScreen(window.localStorage.getItem(guideKey) ? 'game' : 'guide')
  }
  const closeGuide = () => {
  ...
  if (screen === 'title') return <main className="title-screen"><div><h1>察して。</h1><p>空気を読んでください。</p><button className="text-button" onClick={start}>はじめる</button></div><small>Ver. 0.4</small></main>
  if (screen === 'guide') return <main className="guide-screen"><div role="dialog" aria-labelledby="guide-title"><p id="guide-title">気になるものを<br />タップしてください。</p><span className="player-guide">オレンジの服の「あなた」がプレイヤーです</span><p>何もしないのも答えです。</p><button className="text-button" onClick={closeGuide}>わかりました</button></div></main>
