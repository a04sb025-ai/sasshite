...
function Person({ pose = 'standing', shirt = 'sage', label }: { pose?: 'standing' | 'sitting' | 'running' | 'looking'; shirt?: string; label?: string }) {
  return <div className={`character ${pose}`} aria-hidden="true">
    ...
    <span className="neck" /><span className={`body ${shirt}`}><i className="collar" /></span><span className="arm arm-left"><i className="hand" /></span><span className="arm arm-right"><i className="hand" /></span>
...
export function SceneArtwork({ sceneId, acted, onAction }: Props) {
  if (sceneId === 'train') return <section className="art train" aria-label="電車内。自分の隣の座席をバッグがふさぎ、乗客が近づいている">
    ...
    <div className="train-wall"><div className="train-window"><span className="landscape" /><i>優先席</i></div><span className="route-map"><b>この電車</b>●━●━●━●</span><span className="door-edge" /></div>
    <div className="train-floor" />
    <div className="bench"><span className="seat-back back-one"><i /></span><span className="seat-back back-two"><i /></span><span className="seat-back back-three"><i /></span><div className="self"><Person pose="sitting" shirt="rust" label="自分" /></div><Bag moved={acted === 'move-bag'} onClick={() => onAction('move-bag')} /></div>
    <div className="approaching-person"><Person pose="standing" shirt="blue" /><span className="passenger-tote" /><span className="look-mark">…</span><span className="gaze-line" /></div>
  </section>
  ...
  if (sceneId === 'elevator') return <section className="art elevator" aria-label="閉まりかけたエレベーターへ、離れた場所から人が走ってくる">
    ...
    <div className={`elevator-doors ${acted === 'open' ? 'opened' : acted === 'close' ? 'closed' : ''}`}><span className="elevator-interior"><i>3</i></span><span className="door left" /><span className="door right" /><span className="door-gap" /></div>
  ...
  if (sceneId === 'karaage') return <section className="art karaage-scene" aria-label="4人で囲む食卓。手前が自分。中央の大皿には唐揚げが1個だけ残っている">
    ...
    <div className="perspective-table"><span className="glass glass-one" /><span className="glass glass-two" /><span className="glass glass-three" /><span className="empty-plate plate-one"><i /></span><span className="empty-plate plate-two"><i /></span><span className="empty-plate plate-three"><i /></span><span className="self-plate"><i /></span>
  ...
  if (sceneId === 'meeting') return <section className="art meeting" aria-label="4人が参加しているオンライン会議">
    ...
    <div className="call-grid">{['田中', '佐藤', 'あなた', '鈴木'].map((name, index) => <div className={name === 'あなた' ? 'me' : ''} key={name}><span className="room-background"><i /></span><Person pose="standing" shirt={['blue', 'sage', 'rust', 'cream'][index]} /><span>{name}<b>●</b></span></div>)}</div>
  ...
  return <section className="art final-scene" aria-label="お疲れさまでした。画面の端に小さな紙くずが落ちている">
    ...
    <div className="floor-line" /><button className={`paper-trash ${acted === 'trash' ? 'picked' : ''}`} onClick={() => onAction('trash')} aria-label="床に落ちた白い丸められた紙くず"><span><i /><b /><em /></span></button>
  </section>
}
