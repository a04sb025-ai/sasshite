...
describe('train Pixi prototype coordinates', () => {
  ...
  it.each(['lap', 'floor'] as const)('supports Before → %s → reset → re-operation', outcome => {
    expect(resetTrainPlay()).toEqual({ state: 'before', outcome: null })
    expect(beginDropTransition(outcome)).toEqual({ state: 'settling', outcome })
    expect(resetTrainPlay()).toEqual({ state: 'before', outcome: null })
    expect(beginDropTransition(outcome)).toEqual({ state: 'settling', outcome })
  })
